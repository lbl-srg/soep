...
def genPlot(with_plots,compTimLog,simTimLog):
    ...
    plt.title('Compile time split',fontweight='bold')
    plt.plot(compTimLog[:,0],compTimLog[:,1],'r-', label='Total compile time',lw=2)
    plt.plot(compTimLog[:,0],compTimLog[:,4],'bo--', label='Total flatten time',lw=1)
    plt.plot(compTimLog[:,0],compTimLog[:,3],'gx--', label='instantiateModel',lw=1)
    plt.plot(compTimLog[:,0],compTimLog[:,53],'cD--', label='compileCCode',lw=1)
    plt.plot(compTimLog[:,0],compTimLog[:,52],'ms--', label='generateCode',lw=1)
    ...
    plt.ylim([0,600])
    plt.grid()
    fig1.savefig("CompileTime_Max"+str(MaxZon)+"Zone.png")


    fig2 = plt.figure(figsize=(5, 5))
    plt.title('Flatten model time split', fontweight='bold')
    plt.plot(compTimLog[:,0],compTimLog[:,4],'r-', label='Total flatten time',lw=2)
    plt.plot(compTimLog[:,0],compTimLog[:,5],'bo--', label='flatten',lw=1)
    plt.plot(compTimLog[:,0],compTimLog[:,11],'gx--', label='prettyPrintRawFlat',lw=1)
    plt.plot(compTimLog[:,0],compTimLog[:,12],'cD--', label='transformCanonical',lw=1)
    plt.plot(compTimLog[:,0],compTimLog[:,51],'ms--', label='prettyPrintFlat',lw=1)
    plt.legend(loc='best',fontsize='small')
    plt.xlabel('Number of Zones')
    plt.ylabel('Time, second')
    plt.xlim([1,MaxZon])
    plt.ylim([0,250])
    plt.grid()
    fig2.savefig("FlattenTime_Max"+str(MaxZon)+"Zone.png")

    fig3 = plt.figure(figsize=(5, 5))
    plt.title('Parse model, Pack unit', fontweight='bold')
    plt.plot(compTimLog[:,0],compTimLog[:,2],'r-', label='Parse model',lw=2)
    plt.plot(compTimLog[:,0],compTimLog[:,54],'b-', label='Pack unit',lw=2)
    plt.legend(loc='best',fontsize='small')
    plt.xlabel('Number of Zones')
    plt.ylabel('Time, second')
    plt.xlim([1,MaxZon])
    plt.ylim([0,10])
    plt.grid()
    fig3.savefig("ParsePack_Max"+str(MaxZon)+"Zone.png")


    fig4 = plt.figure(figsize=(5, 5))
    plt.title('Toal time: compile vs. simulation',fontweight='bold')
    plt.plot(simTimLog[:,0],simTimLog[:,1],'b-',label='Total simulation time',lw=2)
    plt.plot(compTimLog[:,0],compTimLog[:,1],'r-',label='Total compile time',lw=2)
    plt.legend(loc='best',fontsize='small')
    plt.xlabel('Number of Zones')
    plt.ylabel('Time, second')
    ...
    plt.ylim([0,600])
    plt.grid()
    fig4.savefig("TotalTime_Max"+str(MaxZon)+"Zone.png")
