...
#========================================================
def translate_simulate(BUILDINGS_LIB,Base_class,ClassName):

    timeLog=[]
    # Compile model
    tstart_tr = datetime.now()
    fmu_name = compile_fmu(ClassName,"MultiZone.mo", version=2.0, compiler_options={'extra_lib_dirs':[BUILDINGS_LIB, Base_class]})
    ...
    # Simulate model
    tstart = datetime.now()
    res = Scale_test.simulate(final_time=2592000.0)
    tend = datetime.now()
...
if __name__=="__main__":
    ...
    Base_class = os.path.join(os.getcwd(), "tmp/Buildings/Experimental/ScalableModels/") + "ThermalZones"

    # Copy .mo file from Building library to current working folder
    shutil.copy2(os.path.join(os.getcwd(), "tmp/Buildings/Experimental/ScalableModels/") + "ThermalZones/MultiZone.mo","MultiZone.mo")
    ...
    for i in [ZonNum]:
	print "====== Compiling/Simulating model of "+str((i+1)*1) + "-zones ======"

	# Prepare class name with the update of total number of zones
	ClassName = "Buildings.Experimental.ScalableModels.ThermalZones.MultiZone(nZon="+str((i+1)*1)+")"

	# ******* Compilation/Simulation, record the time *******
	TrTime, SimTime = translate_simulate(BUILDINGS_LIB,Base_class,ClassName)
	# ******* ******* ******* ******* ******* ******* *******
